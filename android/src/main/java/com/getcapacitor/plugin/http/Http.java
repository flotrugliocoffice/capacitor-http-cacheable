...
public class Http extends Plugin {
  ...
  private void http(final PluginCall call, final String httpMethod) {
    /* Deprecated - no permission needed by Android on internal use only files
    https://developer.android.com/training/data-storage
    Boolean shouldCache = call.getBoolean("shouldCache", false);
    if (shouldCache == true) {
      if (getPermissionState(Manifest.permission.WRITE_EXTERNAL_STORAGE) != PermissionState.GRANTED) {
        call.reject("Write permission not allowed");
        return;
      }
      if (getPermissionState(Manifest.permission.READ_EXTERNAL_STORAGE) != PermissionState.GRANTED) {
        call.reject("Read permission not allowed");
        return;
      }
    }
     */
    Runnable asyncHttpCall = new Runnable() {
      ...
      public void run() {
        try {
          JSObject response = HttpRequestHandler.request(call, httpMethod, getContext());
